...
export async function GET() {
  try {
    const base = apiBase();
    // Scoped like the row proxy: the tiles on a mainnet build must count
    // that venue (zero until launch), never the shared testnet history.
    const res = await fetch(`${base}/v1/leaderboard/totals?scope=${leaderboardScope()}`, {
      next: { revalidate: 30 },
    });
    if (!res.ok) {
