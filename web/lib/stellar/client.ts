...
export async function buildTransaction(
  ...
  if (rpc.Api.isSimulationError(simulated)) {
    // Contract reverts surface here (the SDK simulates before signing); decode
    // the `#N` code into a user-facing message, else keep the raw detail.
...
export async function submitTransaction(signedXdr: string): Promise<rpc.Api.GetTransactionResponse> {
  ...
  if (result.status === 'FAILED') {
    ...
    throw new Error(decodeContractError(errorMessage));
  }
