...
- [~] **P4-14** [medium/d] Personal-mode positions/orders via the indexer API with chain-scan fallback; shared getAccount per batch; OrderBook off the global scan (W-2).
  ...
  - [x] **Orders + OrderBook DONE** 2026-07-16 (staging, uncommitted at time of writing). New public `GET /v1/orders/open?trader=&status=open|all` (route in `api/src/routes/orders.ts`, fold in `StatsService.listOrders` — `orders_cache` table was dropped in the Postgres baseline, so the fold runs over `events_raw` at query time, scoped to the current market contract like the leaderboard). Web `fetchOrders` + OrderBook hydrate only the hinted ids via `getOrdersByIds` (parallel), with the whole-market scan kept as fallback. Trust is gated by `web/lib/api/gateway.ts gatewayServesThisMarket()`: `/v1/health` market address must match `NEXT_PUBLIC_MARKET_ID` AND indexer `ledgerAgeSeconds` ≤ 120 — same gate now lets positions trust a successful-EMPTY response (kills the empty-wallet whole-market scan). Also removed the redundant per-read `getAccount` in `buildSimulateTransaction` + `oracle.getPrice` (simulation ignores sequence numbers) — halves every remaining chain read. sdk-ts `orders.open()` added for surface parity; sdk-py pending.
...
- [x] **P4-26** [medium/w] **Leaderboard durable fix (Tier B)** — DONE 2026-07-13 (Supabase migration). Web `/api/leaderboard` now proxies GET /v1/leaderboard (market-scoped, trades/liqCount/updatedAt fields, merged with the `leaderboard_legacy` pre-cutover baseline); the web cron BFS + separate Turso DB + `processed_txs` path are DELETED. Data layer moved Turso→Supabase Postgres via `@noether/db` + `scripts/migrate-turso-to-supabase/` (cutover runbook in that README).
