...
- **…and with a footprint check** — `npx tsx packages/tx-builders/scripts/footprint-check.ts <env> <positionId> <trader> <ASSET> --raw --expect full`
  on a live position proves the upgraded vault/market declare EVERY
  conditional key read-write on their own (the 2026-08-30 stale-footprint
  trap, KNOWN_ISSUES C-4, completed 2026-09-02 for the shortfall books and
  the ADL flag). Run it on a position that is currently LOSING as well as
  one that is winning — the two paths are different code. A pre-fix shape
  (`TotalFees` absent, `ShortfallReserve` read-only) means the wrong WASM
  is live; shortfall keys `absent` on a losing position means the vault
  was upgraded without the market (or the reverse) — vault first, always.
