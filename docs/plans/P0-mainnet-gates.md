...
| L0-9 | Smoothed liquidation/trigger mark (TWAP/median eligibility, fresh settlement) | L | mixed | batch-1-redeploy | L0-7, L0-19 | todo |
| L0-10 | acceptablePrice bound on market open/close | M | mixed | batch-1-redeploy | L0-21 | todo |
| L0-11 | Stop-loss guaranteed execution (kill silent cancel-on-slippage-breach) | S | contracts | batch-1-redeploy | — | todo |
| L0-12 | Per-market margin/leverage ladder (wire the tested risk config) | M | mixed | batch-1-redeploy | — | todo |
| L0-13 | Per-asset funding + real magnitude with clamps (SIP-279) | L | mixed | batch-1-redeploy | L0-12, L0-14 | todo |
| L0-14 | Absolute per-market OI caps + net-skew cap | S | mixed | batch-1-redeploy | L0-12 | todo |
| L0-15 | Pause & emergency semantics (exit-only pause, trader symmetry, timelocked recovery) | M | contracts | batch-1-redeploy | L0-20 | todo |
...
- L0-8 — publisher set grows to 3 keys, `set_quorum` (default 2), `update_quorum_ed25519_persistent` with per-feed median storage, `publisher_prices` view.
...
- L0-9 interim — keeper two-strike trigger confirmation + single-heartbeat spike alert (ships before Batch 1; the on-chain smoothed gate supersedes it, the two-strike stays as simulation-cost hygiene).
